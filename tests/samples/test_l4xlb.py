...
FIXTURES_DIR = os.path.join(os.path.dirname(__file__), '../../samples/x-l4xlb')
...
def test_envgroup_attachment(resources):
    ...
    assert set(a['environment'] for a in attachments) == {'test1', 'test2'}
...
def test_instance_bidge_location_parity(resources):
    ...
    instance_group_mgr = \
    [r['values'] for r in resources if r['type'] == 'google_compute_region_instance_group_manager'][0]
